...
let supabase;
try {
  supabase = createClient(SUPABASE_URL.replace(/\/+$/,''), SUPABASE_ANON_KEY);
} catch(e){
  console.error('createClient error', e);
  setStatus('Ошибка инициализации Supabase (createClient)', 'err');
}
...
async function loadFromSupabase(){
  ...
  try {
    ...
    if (!res.data){
      // строки ещё нет — создадим пустую
    ...
    setStatus('Сетевая ошибка при загрузке (см. консоль)', 'err');
  }
}
...
async function saveNow(){
  ...
  try{
    ...
    setStatus('Сетевая ошибка при сохранении (см. консоль)', 'err');
  }
}

// Сохраняем после кликов
document.addEventListener('click', (e)=>{
  if (e.target.closest('.day')) scheduleSave();
}, {passive:true});

// Ждём появления DOM-календаря, затем грузим
let tries = 0;
